...
Status: **implemented in worktree `fix/issue-8-local-endpoints` (2026-08-11), unreleased** — D1/D3/D4 done, unit tests green; D2 deferred by decision
Related: [Codeberg #8](https://codeberg.org/fitguy/Chompass/issues/8),
[`docs/local/ISSUE_BACKLOG.md`](local/ISSUE_BACKLOG.md)

## Problem statement

The Custom (OpenAI-compatible) provider — and any other user-entered endpoint —
cannot talk to a self-hosted server (reporter: OmniRoute on a home server) in
either mode:
...
- `AndroidCAStore` contains both system and user CAs, so valid public certs
  ...
  store — the reporter's exact mental model ("the app should read the Android
  cert store").

### D2 (decision): LAN cleartext in release — **defer, document**
...
- Device pass (Windows adb, per AGENTS.md workflow): OmniRoute http on debug
