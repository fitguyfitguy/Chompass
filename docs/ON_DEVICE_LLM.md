...
**Status (2026-09-08):** Smoke test **passes** on **Pixel 9a / GrapheneOS** with GPU backend (`litertlm-android` **0.16.1**). Tier A 6/6 `status=ok` (repeat 2, `full` prompt, ~23–29 s); Tier B 4/4 `jsonOk=true` + multi-turn `status=ok`. `ConversationConfig.maxOutputToken=2048` (4096 if MTP). **0.17.0** needs Kotlin 2.4 — not consumed. Recommended daily-driver: **`preset=daily`** (`gpu` + `fewshot_units` + MTP when cache warm).
...
Archived to [`archive/ON_DEVICE_LLM_EXPERIMENTS.md`](archive/ON_DEVICE_LLM_EXPERIMENTS.md).
...
4. **GPU MTP (speculative decoding):** On Pixel 9a with `fewshot_units`, MTP delivers **~1.6× faster Tier A** (~15–17 s vs ~25–30 s) with **no JSON truncation** in Exp 2c (all `status=ok`, unitOptions preserved). Token-budget bug [#2816](https://github.com/google-ai-edge/LiteRT-LM/issues/2816): pin **0.16.1** now sets `maxOutputToken=2048` (4096 if MTP). Device pass 2026-09-08 (Pixel 9a, no MTP): Tier A 6/6 ok, Tier B 4/4 + multi-turn ok. **0.17.0** needs Kotlin 2.4. Default **MTP off** for smoke tests; enable when benchmarking decode speed.
...
- **Version pin:** [`android/gradle/libs.versions.toml`](../android/gradle/libs.versions.toml) → `litertlm = "0.16.1"` (2026-09-08 device pass, Pixel 9a). Latest Maven **0.17.0** exposes the same `maxOutputToken` / `SamplerConfig.temperature` knobs but is compiled with Kotlin 2.4.0 metadata; project compiler is 2.2.0 so it cannot be consumed. `OnDeviceLlmClient.entryConversationConfig` sets `maxOutputToken=2048` (4096 if MTP) and `SamplerConfig(topK=40, topP=0.95, temperature=0.1)`.
...
Archived to [`archive/ON_DEVICE_LLM_EXPERIMENTS.md`](archive/ON_DEVICE_LLM_EXPERIMENTS.md). Outcome: shipped; Tier A (text) and Tier B (photo) run through production dispatch as `AIProvider.ON_DEVICE` behind the default-off Settings toggle (see Production integration above).
