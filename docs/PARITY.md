...
| Progress (weight, BF, measurements, forecast) | shared | `ui/progress/` | `components/progress-*` | FCAST/ADAPT mirrored; interactive chart tips; equal range chips; **configurable default range** (`progressDefaultRangeId`, factory `1W`) with last-viewed precedence; display-only **7-day weight trend** overlay (not Adaptive Goals). **Android-only (2026-08):** `Customize Progress` settings sub-screen + optional per-site body-measurement trend plots (8 sites, off by default, `progressMeasurementSites` empty set; cm/in per height unit; one compact card per enabled site, range-chip filtered): closes Codeberg #18 |
| Settings (profile, goals, units, AI, data) | shared | `ui/settings/` hub + group sub-screens (Profile / Goals & Nutrition / **Food & Entry** / App & Display [+ **Water**, **Notifications**, **Customize Progress** sub-screens] / AI & Speech / Health & Data [+ **Sync** sub-screen]) | `settings-view.js` hub (Personal / Goals / App / AI / Data / About) | Android 2026-08 settings overhaul: **Food & Entry** group (logging, serving sizes, photo analysis), Water + Notifications + Sync as sub-screens, cross-linked rows between related screens (Goals ↔ Water, Water ↔ Notifications, AI ↔ Food & Entry, Data ↔ Sync), hub **Suggestions** card (dismissible nudges toward water/adaptive-goals/Health Connect/backup/notifications; Android-only), danger-zone separation for destructive actions. **PWA still:** 5-group hub; serving units/photo toggles stay under AI; Water stays under App; sync already a subpage. Custom P/C/F pins; browser speech language; Android-only called out. Accent default `system`; primary AI default `gemini`; food-row chips honor fiber when selected. **Android-only:** “Enter custom value” free-entry on every wheel-picked goal (calories, macros, keto net carbs, optional nutrients). **Android-only (2026-08):** appearance modes System / Light / Dark / **OLED** (true-black backgrounds, incl. all 4 Glance widgets; explicit modes also stop the widgets following the system); PWA keeps its own `data-theme` mechanism |
| Onboarding | shared | `ui/onboarding/` | `components/onboarding-*` | Birthday + editable plan-ready macros; BF stored as fraction |
| Food entry (manual, barcode OFF, AI photo/text) | shared | entry flows | entry + `photo-ai-flow` | **Android ahead (2026-08):** lightweight pre-Analyze staging: **text note required by default** before LLM (Skip note & analyze still available; after 3 empty skips offer “Don’t ask for a note again”; Settings → Ask for a photo note); optional label photo + grams; then Analyze morphs into the Log sheet with a ready-gate. Mid-flight tip/add-photo can re-analyze. **PWA still:** multi-photo review + analyze overlay, then FoodResult-like Log. Shared otherwise: **all images** to BYOK; barcode→OFF soft context; progressive phases + streamed partials; quantity/unit picker; constituents; Ask AI to correct; voice; barcode; progressive meal; serving-unit inference. Android-only: heuristic customization, saved-photo reprocess, **relative quantity edits** (`+20` / `-10` deltas in the serving-quantity field). Shared: **2D matrix codes** (QR / Data Matrix, GS1 GTIN extraction, shared `testdata/parity/barcode-codes.json`); live scan ignores non-product frames and keeps scanning until a GTIN-normalizable code is read (shared picker semantics, `docs/local/PLAN_BARCODE_SCAN_RETRY_FIX.md`) |
