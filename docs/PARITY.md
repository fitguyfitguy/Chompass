...
| Food entry (manual, barcode OFF, AI photo/text) | shared | entry flows | entry + `photo-ai-flow` | **Android ahead (2026-08):** photo/gallery/share auto-starts analysis (no obligatory multi-photo review sheet); streaming morphs into the Log sheet with a ready-gate (Log/fields locked until Complete); optional tip chips + grams + add-photo live on that sheet and re-analyze. **PWA still:** multi-photo review + analyze overlay, then FoodResult-like Log. Shared otherwise: **all images** sent to BYOK providers; photo AI **auto-scans barcodes** → OFF soft context; progressive phases + streamed partials when SSE available; quantity + unit picker + live macro scale; editable constituents; edit-entry **Ask AI to correct**; voice; barcode; progressive meal draft; serving-unit inference mode. Android-only: heuristic customization sheet, saved-photo reprocess |
