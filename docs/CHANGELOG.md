...
- **Manual protein, carbs, and fat save as grams** (Android): typing or scrolling 2 on Manual entry now logs 2 g, not 0.2 g. The macro and micro wheels scroll whole grams with a separate tenths wheel, so 80 g is one flick. Calories were already correct. Closes Codeberg [#73](https://codeberg.org/fitguy/Chompass/issues/73) by [@swayevenly](https://codeberg.org/swayevenly).
