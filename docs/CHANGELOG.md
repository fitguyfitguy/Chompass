...
- **Custom caffeine and nicotine drinks** (Android): edit or delete any drink or product, built-ins included, or add your own with a default amount, like a 65 mg espresso or a pouch with its mg per dose. Presets drive the +1 chips on the Add food sheet. A caffeine drink can also carry milk (kind and ml); logging it writes the caffeine tracker row and a milk food in the current meal. Home caffeine totals include tracker mg. Deleting the caffeine row removes the milk food; deleting the milk food keeps the caffeine log. Nicotine stays habit-only. Presets stay on the device. Settings → Trackers → Caffeine or Nicotine. Codeberg [#55](https://codeberg.org/fitguy/Chompass/issues/55) by [@DontBlameMe](https://codeberg.com/DontBlameMe).
...
- **Control how much the model thinks on custom endpoints** (Android): Settings → AI & Speech has a thinking control for custom OpenAI-compatible endpoints and Ollama, with Off, Low, Medium, High and Auto. Auto (the default) sends nothing, so existing setups are unchanged; Off asks the model to skip thinking so the response budget pays for the answer. Follows Codeberg [#97](https://codeberg.org/fitguy/Chompass/issues/97) by [@mik112](https://codeberg.com/mik112).
- **Scanning an unknown barcode offers to add the product** (Android + web): when a barcode has no match in Open Food Facts, the error now has an Add to Open Food Facts action. It opens the Open Food Facts product form in the browser with the barcode in the link, so the next person's scan finds it.
...
- **Higher ceilings for AI response length and read timeout** (Android): the max response length setting now reaches 16000 tokens (was 8000) and the read timeout 900 seconds (was 300), so long replies from local models fit in one answer. Follows Codeberg [#97](https://codeberg.org/fitguy/Chompass/issues/97) by [@mik112](https://codeberg.com/mik112).
...
- **Packaged-food search is leaner on Open Food Facts** (Android): searching packaged foods now uses Open Food Facts' dedicated search engine, which answers in one request and handles typos and brand names better, German ones included. One search is capped at four requests, and when Open Food Facts reports rate limiting, search pauses for a minute instead of piling on. Requests carry the app name, version and a contact address, per their usage policy.
...
- **The response-length error names the fix** (Android): when a model runs out of response length mid-answer, the error now says what to do: raise the max response length or limit thinking, instead of the generic could-not-understand reply. Follows Codeberg [#97](https://codeberg.org/fitguy/Chompass/issues/97) by [@mik112](https://codeberg.com/mik112).
- **Very long lists split into batches instead of failing** (Android): when a long ingredient list still fails after the automatic retries, the app now sends it in batches of four items and merges the answers into one entry with every item's nutrition, instead of failing the whole entry. Follows Codeberg [#97](https://codeberg.org/fitguy/Chompass/issues/97) by [@mik112](https://codeberg.com/mik112).
...
### Fixed
- **Notifications show the app mark in the status bar** (Android): the small icon used the full launcher icon, which Android flattens into a blank tile when it tints. It now uses the plain mark.
...
- **Meals no longer go missing from Health Connect after a hiccup** (Android): if the Health Connect service was briefly unreachable (mid-update, cold start), the mirror write was skipped with no retry, so the diary and Health Connect drifted apart and a reinstall rebuilt an incomplete food log. Unconfirmed writes are now queued and retried on the next sync, retries cannot duplicate a record that did land, and deleting an entry can no longer be undone by a late write. Ports the fix from upstream Fud AI (#204, #312).
- **Your settings edits are not undone by a background refresh** (Android): saving a value such as the water cup size right as the app re-read its settings from storage could snap the row back to an older number. Edits made while the app refreshes now win. Follows Codeberg [#92](https://codeberg.org/fitguy/Chompass/issues/92) by [@joe455555](https://codeberg.org/joe455555).
- **A late time change on the review sheet still logs** (Android): changing Time and tapping Log in one motion could save the earlier time. The picked time now survives further edits and a late change through Log. Follows Codeberg [#93](https://codeberg.org/fitguy/Chompass/issues/93) by [@smg950u](https://codeberg.org/smg950u).
...
- **Re-run works on finished and failed queue entries** (Android): the Play button on an entry in the analysis-queue history did nothing. It now runs that entry again, whether it failed earlier or succeeded. Follows Codeberg [#99](https://codeberg.org/fitguy/Chompass/issues/99) by [@mik112](https://codeberg.org/mik112).
- **Long ingredient lists no longer fail with local AI models** (Android + web): models that reason before answering wrapped their thinking into the reply, which showed as "Could not understand the AI response". Thinking is now stripped from the reply before it is read, and replies that list each ingredient get a higher response-length floor so a long list is not cut short at the default cap. Follows Codeberg [#97](https://codeberg.org/fitguy/Chompass/issues/97) by [@mik112](https://codeberg.org/mik112).
- **On-device AI runs with less free memory** (Android): the safety margin kept in reserve before loading the on-device model was still sized for smaller phones and refused everyday use on 8 GB devices. The margin now scales down with the phone's memory. Follows Codeberg [#46](https://codeberg.org/fitguy/Chompass/issues/46) by [@marcelklehr](https://codeberg.org/marcelklehr) and [@invisibleman](https://codeberg.org/invisibleman).
- **On-device AI works in release builds** (Android): analysis with an on-device model crashed the app in builds from F-Droid or Codeberg, on every model, while the same code built from Android Studio worked. The release optimizer renamed internals that the on-device engine reads by name; those names are now preserved. Follows Codeberg [#101](https://codeberg.org/fitguy/Chompass/issues/101) by [@dadus33](https://codeberg.org/dadus33).
