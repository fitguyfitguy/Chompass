...
- **Intermittent fasting timer** (Android + web): an optional, local-only fasting timer. Start or stop a fast from Home or the Add food sheet, and set your own fasting and eating windows freely (with quick picks for the common protocols: 12:12, 14:10, 16:8, 18:6, 20:4, 23:1). The fasting bar on Home shows how long until you can eat while fasting, and how long until the next fast starts while your eating window is open. An optional Auto fast windows mode runs the cycle for you: your fast starts automatically at the fast start time you choose (default 20:00, shown on Home as "Fast starts at …" / "Next fast at …") and ends at the goal, with no buttons on Home (you can still start one manually). Two optional reminders, both with a lead time you choose (0–120 minutes, off by default): a start nudge before the fast starts (works with auto windows too), and a break-fast nudge before your fast reaches its goal. The timer survives restarts and stays on your device: nothing is synced, exported, or included in shared meals. Coach knows how long you have been fasting when you ask about eating timing. Long-press the app icon for a Start or stop fasting shortcut when the timer is on. Off by default under Settings → Trackers & Reminders → Fasting.
- **Optional nicotine tracking** (Android + web): log cigarettes, vapes, pouches and more with quick +1 chips or a custom count (with optional nicotine in mg), see today's total against a daily limit you set, and edit or delete any entry from a per-day history. Everything stays on your device and in your WebDAV sync; nothing is sent to an AI provider. Off by default under Settings → App & Display → Nicotine.
- **Caffeine tracking** (Android + web): coffee, tea, and energy drinks now carry their caffeine, with a daily total and an adjustable daily max (default 400 mg) on your Home cards and in Goals & Nutrition. Caffeine from photo, text, barcode, and manual logging is kept, can be edited, and is included in diary exports, shared meals, sync, and Health Connect. Closes Codeberg [#55](https://codeberg.org/fitguy/Chompass/issues/55) by [@DontBlameMe](https://codeberg.org/DontBlameMe).
- **Caffeine tracker** (Android + web): a water-style tracker on top of the caffeine nutrient. Turn it on under Settings → Trackers & Reminders → Caffeine and Home shows today's caffeine (tracker logs plus caffeine from food entries) against the daily limit you set, the Add food sheet gains +1 chips for coffee, tea, and energy drinks, and each logged cup lands in a per-day history you can edit or delete. Off by default.
