...
- **Intermittent fasting timer** (Android + web): an optional, local-only fasting timer. Start, stop or cancel a fast from Home or the Add food sheet, set an optional goal length (for example 16 hours) with quick picks for the common protocols (12:12, 14:10, 16:8, 18:6, 20:4, 23:1), and get one notification when you reach it. Home shows how long is left until your eating window opens. An optional daily start reminder (off by default) nudges you to begin your next fast at a time you choose, and is skipped while a fast is already running. The timer survives restarts and stays on your device: nothing is synced, exported, or included in shared meals. Coach knows how long you have been fasting when you ask about eating timing. Long-press the app icon for a Start or stop fasting shortcut when the timer is on. Off by default under Settings → Trackers & Reminders → Fasting.
