...
- **Custom caffeine and nicotine presets** (Android): rename the built-in drinks and products, or add your own with a default amount, like a 65 mg espresso or a pouch with its mg per dose. Presets drive the +1 chips on the Add food sheet, reorder them to taste, and renames apply to old entries too. Deleting a preset moves its entries to Other and keeps every amount. Presets stay on the device; entries keep syncing as before, and a device without the preset shows them under Other. Settings → Trackers → Caffeine or Nicotine. Codeberg [#55](https://codeberg.org/fitguy/Chompass/issues/55) by [@DontBlameMe](https://codeberg.org/DontBlameMe).
