...
- **Recalculate explains itself and uses your full history with cloud AI** (Android): the one-line result is now a result sheet showing your new calorie and macro targets, which AI provider and model set them (and when one failed and a backup model answered instead), whether it used the smart or the conservative path, the formula chain behind the number (resting burn, activity, goal pace), the data it looked at (weigh-ins, logged intake, your implied maintenance or why it was set aside), the previous goal next to the new one, and the model's full reasoning. Cloud providers get your raw weigh-in series and the last 14 days of intake and judge the trend themselves, while the on-device model stays conservative until your history is dense enough, and the app still falls back to the built-in formula or your measured Health Connect burn when the logs are too thin.
