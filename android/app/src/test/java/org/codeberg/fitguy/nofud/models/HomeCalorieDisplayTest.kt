...
import org.codeberg.fitguy.nofud.services.health.ActivityDataSource
...
class HomeCalorieDisplayTest {
    ...
    @Test
    fun effectiveMode_netFallsBackWithoutMeasured() {
        val estimated = ResolvedActiveBurn(400, ActiveCalorieSource.ESTIMATED)
        assertEquals(
            HomeCalorieDisplayMode.STATIC,
            HomeCalorieDisplay.effectiveMode(HomeCalorieDisplayMode.NET, estimated)
        )
        assertEquals(
            HomeCalorieDisplayMode.NET,
            HomeCalorieDisplay.effectiveMode(
                HomeCalorieDisplayMode.NET,
                ResolvedActiveBurn(400, ActiveCalorieSource.MEASURED),
            )
        )
    }
    ...
    @Test
    fun effectiveMode_dual_usesEstimatedWhenNoHc() {
        val estimated = ResolvedActiveBurn(400, ActiveCalorieSource.ESTIMATED)
        assertEquals(
            HomeCalorieDisplayMode.DUAL,
            HomeCalorieDisplay.effectiveMode(HomeCalorieDisplayMode.DUAL, estimated)
        )
    }
