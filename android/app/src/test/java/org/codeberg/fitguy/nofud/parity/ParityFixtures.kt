 ...
 * Path is injected by Gradle (`nofud.parity.dir`) so unit tests do not depend on CWD.
 */
object ParityFixtures {
    fun dir(): File {
        val fromProp = System.getProperty("nofud.parity.dir")
        if (!fromProp.isNullOrBlank()) {
            val f = File(fromProp)
            require(f.isDirectory) { "nofud.parity.dir is not a directory: $fromProp" }
            return f
        }
        ...
        error("Could not locate testdata/parity (set nofud.parity.dir)")
    }
