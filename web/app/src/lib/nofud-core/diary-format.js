 ...
 * compatible with android/app/src/main/java/org/codeberg/fitguy/nofud/export/
...
export function exportDiary({ entries, targets, dateRange }) {
  ...
  return {
    export: { app: "NoFUD", format_version: DIARY_FORMAT_VERSION, date_range: dateRange },
    days,
  };
}
...
export function importDiary(doc, idGen = () => crypto.randomUUID()) {
  ...
  if (app !== "nofud" && app !== "fud ai") throw new UnsupportedFormatError(`unrecognized app "${exp.app}"`);
